...
class Invited
{
    ...
    public static function _setInvitedForTheQueueRandom($totalInvitedQueue, $database) {
        ...
        echo "query invited now <br>";
		$timezone_except = "'AEDT','WIB', 'A', 'ACDT', 'AEST', 'WITA', 'AWST'";
        $timezone_only = "'CET','CST', 'EST'";
        self::$invitedForTheQueue = $database->selectV1('fs_invited', '*', "invited_status not in(2,3,7,12,4) and timezone in($timezone_only) ORDER BY invited_id desc limit $totalInvitedQueue");
        // self::$invitedForTheQueue = $database->selectV1('fs_invited', '*', " invited_id = 35");
       // print_r(self::$invitedForTheQueue);
        echo "total new added total = " . count(self::$invitedForTheQueue) . '<br>';
    }

...
foreach ($invitedQueue->getAllInvitedInTheQueueWhoWillReceiveAnInviteEmail_() as $queueInvited)
{
    ...
    if($invited->getTotalEmailSent_() <= 2)
    {
        ...
        if (Time::getTimeZoneDateTime24() >= $dateTime)
        {
            ...
            if(Email::sendInviteEmail($admin->getAdminEmail_(), $invited->getEmail(), "Invited", $qId, $invited->getFirstName()) ) {
                // Email::sendInviteEmail($admin->getAdminEmail_(), 'mrjesuserwinsuarez@gmail.com', "Invited", $qId, $invited->getFirstName());



                //                $alert->alert("\nSend invitation",
                //
                //                    $email, "Fs Invited", $qId, $invited->getFirstName()),
                //                    " sending invitation set admin email = " . $admin->getAdminEmail_() .
                //                    " invited email = " . $invited->getEmail() .
                //                    " qid = " . $qId .
                //                    " first name = " . $invited->getFullName()
                //                );
                // Email::sendToEmail('admin@gmail.com', "Fs invitation", "content of the email", "invited@gmail.com", "Fs Invitation");
                ...
                $alert->alert("\nUpdate total send email ",
                    $invited->updateTotalEmailSent($iId, $invited->getTotalEmailSent_() + 1, $database),
                    "set iId = $iId and last total email sent  " . $invited->getTotalEmailSent_() . " new total email sent " . $newTotalSent
                );
            }
            else
            {
        ...
        {
            echo "\n time zone is less than with date time zone";
        }
    }
    else
    {
